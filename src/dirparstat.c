...
pardirstat(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    */
    Py_XINCREF(path.object);

 /* POSIX */
    struct dirent **namelist;
    STRUCT_STAT *sts=NULL;
    int imax;
    Py_BEGIN_ALLOW_THREADS
        imax = par_dir_stat(path.narrow, &namelist, &sts);
    Py_END_ALLOW_THREADS
    if(imax==0) goto error;
    PyObject *res = PyDict_New();
    for(int i=0; i< imax; i++){
        if(strcmp(namelist[i]->d_name, "..") == 0)
            continue;
        DirEntry *entry = (DirEntry *)DirEntry_from_posix_info(&path, namelist[i]->d_name,
                                         strlen(namelist[i]->d_name), namelist[i]->d_ino
#ifdef HAVE_DIRENT_D_TYPE
                                            , namelist[i]->d_type
#endif
                                            );
        entry->stat = _pystat_fromstructstat(&sts[i]);
        free(namelist[i]);

        PyDict_SetItem(res, entry->path, (PyObject *)entry);
        Py_DECREF((PyObject *)entry);
    }
    free(sts);
    free(namelist);
    Py_XDECREF(path.object);
    if (imax <= 0) {
        path_error(&path);
        goto error;
    }
...
mtimelevel(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O&i:mtimelevel", keywords,
                                     path_converter, &path, &level))
        goto error;
 ...
 /* POSIX */
    ...
    struct fts_data res={level, 0, 0};
    const char *top[2];
    top[0] = path.narrow;
    top[1] = (char *)NULL;
    if(fts_run((char * const *)top, level, ranger_maxmtime, &res) != 0){
        goto error;
    };

    PyObject *r = PyFloat_FromDouble(res.max_mtime);
    return r;
...
get_cumulative_size(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O&:get_cumulative_size", keywords,
                                     path_converter, &path))
        goto error;
 ...
 /* POSIX */
    ...
    top[0] = path.narrow;
    top[1] = (char *)NULL;
    if(fts_run((char * const *)top, 0, ranger_cumsize, &res) != 0){
        goto error;
    };

    PyObject *r = PyLong_FromLong(res.size);
    return r;
...
static PyMethodDef scandir_methods[] = {
    {"par_dir_stat",         (PyCFunction)pardirstat,
                        METH_VARARGS | METH_KEYWORDS,
                        posix_scandir__doc__},
    {"mtimelevel",         (PyCFunction)mtimelevel,
                        METH_VARARGS | METH_KEYWORDS,
                        posix_scandir__doc__},
...
/* Local Variables: */
/* company-clang-arguments: ("-I/usr/include/python3.6m") */
